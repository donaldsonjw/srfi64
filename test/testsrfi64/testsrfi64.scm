...
(define (srfi64test-error reason . args)
   (error "srfi64 tests" reason args))
  ...
  (let ((r (test-runner-null))
    ...
    (test-runner-on-bad-count!
     ...
     (lambda (runner count expected-count)
       (srfi64test-error (string-append "bad count " (number->string count)
			     " but expected "
			     (number->string expected-count)))))
    (test-runner-on-bad-end-name!
     r
     (lambda (runner begin end)
       (srfi64test-error (string-append "bad end grojup name " end
			     " but expected " begin))))
    (test-runner-on-test-end! 
     r 
     (lambda (runner)
